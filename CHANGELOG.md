...
- CodeGraph now indexes **FEGate SL** (FFS Script Language) — `.sl` scripts (functions, structs, enums, `foreach_db` loops, and `::` scoped calls) plus FEGate API headers, so engine API calls like `dbNode::First()` resolve from your script straight to the API function that declares them.
- FEGate SL projects no longer need a copy of the API headers in every project: drop a `.codegraph/sl_root_cache.txt` listing the absolute paths of your shared `fegate_api` (and example) folders, then run `codegraph index --force` — CodeGraph indexes those declarations into your project's graph so `dbNode::First()`-style calls resolve to the API without carrying the headers around. Re-run `index --force` after editing the cache file.
- `codegraph status` (and the `codegraph_status` tool) now show an **SL (FEGate) Wiring** section — native `.sl` script nodes, `.h` API-stub nodes, the edges among them, and how many calls resolved to an API declaration — so you can see the SL subgraph's wiring at a glance. It only appears on projects that contain SL.
