...
Distributed as `@evespimrose/codegraph` on npm; same binary serves as installer, indexer, and MCP server.
...
`src/installer/` is the entry point for `codegraph install` (and the bare `codegraph`/`npx @evespimrose/codegraph` invocation). Architecture:
