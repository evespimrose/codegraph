...
print(json.dumps({
  'hookSpecificOutput': {
    ...
    'permissionDecisionReason': f'Sonar Protocol L1: PowerShell로 Bash 탐색 우회 시도 감지 ({violation}) [{ctx}]',
    'additionalContext': f'⛔ SONAR GUARD L1: PowerShell 우회 차단\n\n감지: {violation} [{ctx}]\n\nBash와 동일한 규칙 적용:\n  1. codegraph_context / codegraph_search\n  2. codegraph_files (파일 구조)\n  3. Glob / Grep 도구 (단, codegraph 후)\n\nPowerShell은 시스템 작업(설치, 빌드, 권한)에만 사용 — 파일 탐색/소스 출력 금지'
  }
}))
