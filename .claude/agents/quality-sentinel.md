...
규모 판단 (소형 / 중·대형)
  ...
  → 위반 발견:
      ...
      (최대 3회 반복 후에도 실패 → 사용자 에스컬레이션)
```

### Phase 3 — 최종 보고
두 검증 모두 통과 시 사용자에게 클린 보고:
...
- RIPER 위반을 사용자 모르게 통과 처리 — 모든 위반은 사용자 보고
- 3회 반복 후에도 컨벤션 미통과 시 자의적 통과 처리 — 반드시 사용자 에스컬레이션
