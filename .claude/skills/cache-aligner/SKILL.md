---
name: cache-aligner
description: Use when editing SessionStart hooks (session-start.sh, post-compact.sh) or verifying prompt-prefix KV-cache stability (HR5) — "캐시 정렬 검증", "prefix churn 확인", "cache-aligner". Do NOT use for non-SessionStart hooks or output-side tokens (use output-arm).
---
...
- session-start.sh·post-compact.sh 외 파일 — 본 스킬은 SessionStart prefix 불변식 전용.
- 출력측 토큰 절감 — `output-arm`(직교 축).
