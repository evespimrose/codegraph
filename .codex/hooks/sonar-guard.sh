...
if [ -n "$VIOLATION" ]; then
  python3 -c "
import json
msg = '''⛔ SONAR GUARD: bash 탐색 감지 ($VIOLATION)

Sonar Protocol 위반 — bash로 파일을 탐색하지 말 것.
...
print(json.dumps({
  'hookSpecificOutput': {
    ...
    'permissionDecisionReason': 'Sonar Protocol: bash 탐색 감지 (' + '$VIOLATION' + '). dictionary.md § 1 또는 Glob/Grep 도구를 사용하세요.',
