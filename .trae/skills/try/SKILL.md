...
**⚠️ WRITE PERMISSION GRANTED**: This skill has explicit write permission to create and save documentation files to `docs/try/Try_*.md`. No code file modification allowed, only documentation writing.
...
## 탐색 규약 (Sonar Protocol — DEADLY · L1 Hook 강제)
...
❌ L1 hook이 차단 (try도 예외 아님):
  ...
  - bash find/grep -r/ls -r/rg/fd                  → sonar-guard.sh
  - PowerShell -Recurse                            → sonar-guard-powershell.sh
